...
IF NOT "%firstentry%"=="0" ECHO Manufacturer,Model,Codename,Model Number,Carrier,IMEI,Serial Number,Barcode,HW,ICCID,IMSI,MDN,Custody,Date,Owner,Transfer Date,Additional info>>Device_Inventory.csv
REM ---FOR -------------REFERENCE--   %brand%,%model%,%codename%,%sku%,%carrier%,'%imei%',%serial%,%barcode%,%hardware%,'%iccid%','%imsi%','%mdn%',%custody%,%date%,%owner%,%transfer%,%comments%
IF "%firstentry%"=="0" (
...
REM ICCID ----13 (ICCID)-samsung
FOR /F "skip=1 tokens=2,3,4,5,6,7,8,9 delims='.)" %%i IN ('adb shell service call iphonesubinfo 13') DO <NUL SET /P result=%%i%%j%%k%%l%%m%%n%%o%%p>>temp.txt
IF EXIST temp.txt FOR /F "tokens=1 delims= " %%i IN (temp.txt) DO SET iccid3=%%i
IF EXIST temp.txt del temp.txt 2>&1>NUL

REM IMSI -----iphonesubinfo 7 (IMSI)-lge
...
REM : MDN ----iphonesubinfo 17 (MDN)-moto, samsung
...
REM : MDN ----iphonesubinfo 18 (MDN)-lge
...
REM : MDN ----iphonesubinfo 19 (MDN)-google
FOR /F "skip=1 tokens=2,3,4,5,6,7,8,9 delims='.)" %%i IN ('adb shell service call iphonesubinfo 19') DO <NUL SET /P result=%%i%%j%%k%%l%%m%%n%%o%%p>>temp.txt
IF EXIST temp.txt FOR /F "tokens=1 delims= " %%i IN (temp.txt) DO SET mdn3=%%i
IF EXIST temp.txt del temp.txt 2>&1>NUL


REM change nuences by manufacture
IF "%brand%"=="google" SET "iccid=%iccid2%"
IF "%brand%"=="google" SET "mdn=%mdn3%"

IF "%brand%"=="samsung" SET "iccid=%iccid3%"
IF "%brand%"=="samsung" SET "mdn=%mdn1%"

IF "%brand%"=="motorola" SET "iccid=%iccid1%"
IF "%brand%"=="motorola" SET "mdn=%mdn1%"

REM STILL NEED TO ADD:::::
IF "%brand%"=="lge" SET "iccid="
IF "%brand%"=="lge" SET "mdn="
...
CHOICE /C 1234 /T 4 /D 4 /N /M "Who is owner?"
...
:output
ECHO %brand%,%model%,%codename%,%sku%,%carrier%,'%imei%',%serial%,%barcode%,%hardware%,'%iccid%','%imsi%','%mdn%',%custody%,%date%,%owner%,%transfer%,%comments%>>Device_Inventory.csv
